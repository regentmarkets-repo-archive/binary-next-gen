...
import MarketSubmarketPickerContainer
    from '../asset-picker/MarketSubmarketPickerContainer';
...
export default class TradingTimesCard extends PureComponent {
    ...
    updateTradingTimes = (e: SyntheticEvent) => {
        ...
        if (
            /\d{4}-\d{1,2}-\d{1,2}/.test(inputVal) &&
            inputVal.slice(0, 4) >= 1000
        ) {
            ...
            <div className="trading-times-card">
                ...
                <div className="trading-times-data scrollable">
                    <TradingTimesTable
                        {...this.props}
                        key={filter}
                        times={tradingTimes.filter(a =>
                            this.assetMatchFilter(a.symbol, filter),
                        )}
                    />
                </div>
            </div>
