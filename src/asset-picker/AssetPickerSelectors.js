...
const doesMatchFilter = (asset, filter, watchlist) =>
    ...
    (filter.get('filter') === 'favorites' &&
        watchlist.has(asset.get('symbol')));
...
const doesMatchQueryAndFilter = (asset, filter, watchlist) =>
    doesMatchFilter(asset, filter, watchlist) &&
    doesMatchQuery(asset, filter, watchlist);
...
export const shownAssetsSelector = createSelector(
    ...
    (availableAssets, filter, watchlist) =>
        availableAssets.filter(asset =>
            doesMatchQueryAndFilter(asset, filter, watchlist),
        ),
);
