export default {
    "All": [
        null,
        "Wszystko"
    ],
    "Property": [
        null,
        "Waciwo"
    ],
    "Value": [
        null,
        "Warto"
    ],
    "Balance": [
        null,
        "Saldo"
    ],
    "Trade": [
        null,
        "Zakad"
    ],
    "Watchlist": [
        null,
        "Obserwowane"
    ],
    "Open Positions": [
        null,
        "Otwarte pozycje"
    ],
    "Profit Table": [
        null,
        "Tabela zyskw"
    ],
    "Statement": [
        null,
        "Stan konta"
    ],
    "Settings": [
        null,
        "Ustawienia"
    ],
    "Sign Out": [
        null,
        "Wyloguj"
    ],
    "View": [
        null,
        "Widok"
    ],
    "Ref.": [
        null,
        "Ref."
    ],
    "Purchase": [
        null,
        "Kup"
    ],
    "Indicative": [
        null,
        "Orientacyjna"
    ],
    "Purchase Date": [
        null,
        "Data zakupu"
    ],
    "Purchase Price": [
        null,
        "Cena zakupu"
    ],
    "Sale Date": [
        null,
        "Data sprzeday"
    ],
    "Sale Price": [
        null,
        "Cena sprzeday"
    ],
    "Profit/Loss": [
        null,
        "Zysk/strata"
    ],
    "Total": [
        null,
        "Cakowity"
    ],
    "Location": [
        null,
        "Lokalizacja"
    ],
    "Update": [
        null,
        "Aktualizuj"
    ],
    "Details": [
        null,
        "Szczegy"
    ],
    "Trading Limits": [
        null,
        "Limity handlowe"
    ],
    "Item": [
        null,
        "Element"
    ],
    "Limit ({currency})": [
        null,
        "Limit ({currency})"
    ],
    "Maximum number of open positions": [
        null,
        "Maksymalna liczba otwartych pozycji"
    ],
    "Maximum account cash balance": [
        null,
        "Maksymalne saldo gotwki na koncie"
    ],
    "Maximum daily turnover": [
        null,
        "Maksymalny dzienny obrt"
    ],
    "Maximum aggregate payouts on open positions": [
        null,
        "Maksymalne cakowite wypaty dla pozycji otwartych"
    ],
    "Withdrawal limits": [
        null,
        "Limity wypat"
    ],
    "Your withdrawal limit is {limit} (or equivalent in other currency).": [
        null,
        "Twj limit wypat wynosi {limit} (lub jego ekwiwalent w innej walucie)."
    ],
    "You have already withdrawn the equivalent of EUR {drawn}.": [
        null,
        "Wypacono ju rwnowarto {drawn} EUR."
    ],
    "An additional password can be used to restrict access to the cashier.": [
        null,
        "Moliwe jest uycie dodatkowego hasa, aby ograniczy dostp do sekcji Kasjer."
    ],
    "Sign In": [
        null,
        "Zaloguj"
    ],
    "Get your API token": [
        null,
        "Pobierz token API"
    ],
    "Create Account": [
        null,
        "Utwrz konto"
    ],
    "Action": [
        null,
        "Czynno"
    ],
    "Credit/Debit": [
        null,
        "Winien/Ma"
    ],
    "Purchase Time": [
        null,
        "Godzina zakupu"
    ],
    "Back": [
        null,
        "Poprzedni"
    ],
    "Place Order": [
        null,
        "Z zlecenie"
    ],
    "{asset} will {type} over next {duration}": [
        null,
        "{asset} zanotuje {type} w cigu {duration}"
    ],
    "Price": [
        null,
        "Cena"
    ],
    "Asset": [
        null,
        "Aktywa"
    ],
    "Type": [
        null,
        "Typ"
    ],
    "Ticks": [
        null,
        "Najmniejsze zmiany ceny"
    ],
    "Amount": [
        null,
        "Kwota"
    ],
    "Trade Confirmation": [
        null,
        "Potwierdzenie zawarcia zakadu"
    ],
    "Entry spot": [
        null,
        "Pozycja wejciowa"
    ],
    "Exit spot": [
        null,
        "Punkt wyjciowy"
    ],
    "Next": [
        null,
        "Nastpny"
    ],
    "Open Account": [
        null,
        "Otwrz konto"
    ],
    "Upgrade to Real Money Account": [
        null,
        "Zamie na konto z prawdziwymi pienidzmi"
    ],
    "Date of birth": [
        null,
        "Data urodzenia"
    ],
    "Home Address": [
        null,
        "Adres domowy"
    ],
    "Security": [
        null,
        "Bezpieczestwo"
    ],
    "Spot": [
        null,
        "Cena spot"
    ],
    "Change": [
        null,
        "Zmiana"
    ],
    "Chart": [
        null,
        "Wykres"
    ],
    "Purchase For": [
        null,
        "Kup za"
    ],
    "Rise": [
        null,
        "Wzrost"
    ],
    "Stake": [
        null,
        "Stawka"
    ],
    "Payout": [
        null,
        "Wypata"
    ],
    "Deposit": [
        null,
        "Wpata"
    ],
    "Transactions": [
        null,
        "Transakcje"
    ],
    "Opens": [
        null,
        "Otwarcie"
    ],
    "Closes": [
        null,
        "Zamknicie"
    ],
    "Personal": [
        null,
        "Osobiste"
    ],
    "General": [
        null,
        "Oglne"
    ],
    "Trading Times": [
        null,
        "Godziny handlu"
    ],
    "Asset Index": [
        null,
        "Indeks aktyww"
    ],
    "Today": [
        null,
        "Dzisiaj"
    ],
    "Yesterday": [
        null,
        "Wczoraj"
    ],
    "Tomorrow": [
        null,
        "Jutro"
    ],
    "Takeout": [
        null,
        "Wypata"
    ],
    "News": [
        null,
        "Wiadomoci"
    ],
    "Resources": [
        null,
        "Zasoby"
    ],
    "Last 7 Days": [
        null,
        "Ostatnie 7 dni"
    ],
    "Last 30 Days": [
        null,
        "Ostatnie 30 dni"
    ],
    "Daily Report": [
        null,
        "Raport dzienny"
    ],
    "Up/Down": [
        null,
        "Gra/D"
    ],
    "Touch/No Touch": [
        null,
        "Osignie/Nie osignie"
    ],
    "Ends In/Out": [
        null,
        "Zakoczy si w/poza"
    ],
    "Stays In/Goes Out": [
        null,
        "Pozostanie w/poza"
    ],
    "Settles": [
        null,
        "Rozliczenie"
    ],
    "Language": [
        null,
        "Jzyk"
    ],
    "Color Theme": [
        null,
        "Motyw kolorystyczny"
    ],
    "Light": [
        null,
        "Jasny"
    ],
    "Dark": [
        null,
        "Ciemny"
    ],
    "Name": [
        null,
        "Imi"
    ],
    "Date Of Birth": [
        null,
        "Data urodzenia"
    ],
    "Country Of Residence": [
        null,
        "Pastwo zamieszkania"
    ],
    "Email": [
        null,
        "E-mail"
    ],
    "There are no transactions for selected period": [
        null,
        "Brak transakcji w wybranym okresie"
    ],
    "Sell": [
        null,
        "Sprzedaj"
    ],
    "Buy": [
        null,
        "Kup"
    ],
    "Digit Even": [
        null,
        "Cyfra parzysta"
    ],
    "Touches": [
        null,
        "Osignie"
    ],
    "Digit Over": [
        null,
        "Cyfra wiksza"
    ],
    "Digit Match": [
        null,
        "Cyfra zgadza si"
    ],
    "Digit Under": [
        null,
        "Cyfra mniejsza"
    ],
    "Digit Odd": [
        null,
        "Cyfra nieparzysta"
    ],
    "Digit Differs": [
        null,
        "Cyfra nie zgadza si"
    ],
    "No Touch": [
        null,
        "Nie osignie"
    ],
    "Fall": [
        null,
        "Spadek"
    ],
    "Asian Down": [
        null,
        "Azjatyckie  Spadek"
    ],
    "Asian Up": [
        null,
        "Azjatyckie  Wzrost"
    ],
    "Search For Assets": [
        null,
        "Szukaj aktyww"
    ],
    "Start times": [
        null,
        "Czas rozpoczcia"
    ],
    "Durations": [
        null,
        "Czas trwania"
    ],
    "Payout/Stake": [
        null,
        "Wypata/Stawka"
    ],
    "Purchase for": [
        null,
        "Kup za"
    ],
    "payout": [
        null,
        "wypata"
    ],
    "stake": [
        null,
        "stawka"
    ],
    "Self Exclusion": [
        null,
        "Samodzielne wyczenie"
    ],
    "Cashier Lock": [
        null,
        "Blokowanie sekcji Kasjer"
    ],
    "Change Password": [
        null,
        "Zmie haso"
    ],
    "Password": [
        null,
        "Haso"
    ],
    "Tick Trade": [
        null,
        "Zakad oparty na najmniejszej zmianie ceny"
    ],
    "Full Trade": [
        null,
        "Peny zakad"
    ],
    "Address": [
        null,
        "Adres"
    ],
    "Town/City": [
        null,
        "Miasto"
    ],
    "State/Province": [
        null,
        "Region/Wojewdztwo"
    ],
    "Telephone": [
        null,
        "Telefon"
    ],
    "Postal Code / ZIP": [
        null,
        "Kod pocztowy"
    ],
    "Cashier password": [
        null,
        "Haso do sekcji Kasjer"
    ],
    "Re-enter your password": [
        null,
        "Powtrz haso"
    ],
    "Current password": [
        null,
        "Obecne haso"
    ],
    "New password": [
        null,
        "Nowe haso"
    ],
    "Verify new password": [
        null,
        "Zweryfikuj nowe haso"
    ],
    "Daily turnover limit": [
        null,
        "Maksymalny dzienny limit obrotw"
    ],
    "Once this limit is reached, you may no longer deposit.": [
        null,
        "Po osigniciu tego limitu nie mona dokonywa dalszych wpat."
    ],
    "Maximum aggregate contract purchases per day.": [
        null,
        "Maksymalne dzienne czne zakupy kontraktw."
    ],
    "Daily limit on losses": [
        null,
        "Dzienny limit strat"
    ],
    "Maximum aggregate loss per day.": [
        null,
        "Maksymalna czna strata dzienna."
    ],
    "7-day turnover limit": [
        null,
        "7-dniowy limit obrotw"
    ],
    "Maximum aggregate contract purchases over a 7-day period.": [
        null,
        "Maksymalne czne 7-dniowe zakupy kontraktw."
    ],
    "7-day limit on losses": [
        null,
        "7-dniowy limit strat"
    ],
    "Maximum aggregate loss over a 7-day period.": [
        null,
        "Maksymalna czna strata w okresie 7-dniowym."
    ],
    "30-day turnover limit": [
        null,
        "30-dniowy limit obrotw"
    ],
    "Maximum aggregate contract purchases over a 30-day period.": [
        null,
        "Maksymalne czne zakupy kontraktw w okresie 30 dni."
    ],
    "30-day limit on losses": [
        null,
        "30-dniowy limit strat"
    ],
    "Maximum aggregate loss over a 30-day period.": [
        null,
        "Maksymalna czna strata w okresie 30-dni."
    ],
    "Session duration limit, in minutes": [
        null,
        "Limit czasu sesji, w minutach"
    ],
    "You will be automatically logged out after such time.": [
        null,
        "Po tym czasie nastpi automatyczne wylogowanie."
    ],
    "Exclude me from the website until": [
        null,
        "Wycz mnie z portalu do dnia"
    ],
    "Limits": [
        null,
        "Limity"
    ],
    "Please enter date in the format YYYY-MM-DD.": [
        null,
        "Wpisz dat w formacie RRRR-MM-DD."
    ],
    "New Trade": [
        null,
        "Nowy zakad"
    ],
    "Trade Type": [
        null,
        "Rodzaj zakadu"
    ],
    "Random 100 Index": [
        null,
        "Indeks losowy 100"
    ],
    "Article": [
        null,
        "Artyku"
    ],
    "Start Later": [
        null,
        "Rozpocznij pniej"
    ],
    "Buy Price": [
        null,
        "Cena zakupu"
    ],
    "Earning (%)": [
        null,
        "Dochd (%)"
    ],
    "Profit": [
        null,
        "Zysk"
    ],
    "Entry Price": [
        null,
        "Cena wejciowa"
    ],
    "Exit Price": [
        null,
        "Cena wyjciowa"
    ],
    "Indicative Price": [
        null,
        "Cena orientacyjna"
    ],
    "Potential Profit": [
        null,
        "Potencjalny zysk"
    ],
    "Sell at Market": [
        null,
        "Sprzedaj na rynku"
    ],
    "Go back": [
        null,
        "Wr"
    ],
    "You lost ": [
        null,
        "Poniesiono strat "
    ],
    "Close": [
        null,
        "Zamknij"
    ],
    "Low": [
        null,
        "Niski"
    ],
    "High": [
        null,
        "Wysoki"
    ],
    "Open": [
        null,
        "Otwrz"
    ],
    "Date": [
        null,
        "Data"
    ],
    "Upcoming Events": [
        null,
        "Nadchodzce wydarzenia"
    ],
    "Win": [
        null,
        "Wygrana"
    ],
    "Loss": [
        null,
        "Strata"
    ],
    "Full Screen": [
        null,
        "Peny ekran"
    ],
    "Start Now": [
        null,
        "Rozpocznij teraz"
    ],
    "Daily Prices": [
        null,
        "Ceny dzienne"
    ],
    "Spreads": [
        null,
        "Spready"
    ],
    "Confirm new password": [
        null,
        "Potwierd nowe haso"
    ],
    "TICK": [
        null,
        "NAJMNIEJSZA ZMIANA CENY"
    ],
    "Minutes": [
        null,
        "Minuty"
    ],
    "Seconds": [
        null,
        "Sekundy"
    ],
    "Hours": [
        null,
        "Godziny"
    ],
    "Days": [
        null,
        "Dni"
    ],
    "N/A": [
        null,
        "Nie dotyczy"
    ],
    "Duration": [
        null,
        "Czas trwania"
    ],
    "Start Time": [
        null,
        "Godzina rozpoczcia"
    ],
    "Now": [
        null,
        "Teraz"
    ],
    "Later": [
        null,
        "Pniej"
    ],
    "return": [
        null,
        "zwrot"
    ],
    "Portfolio": [
        null,
        "Portfolio"
    ],
    "Video": [
        null,
        "Nagranie"
    ],
    "Digit Stats": [
        null,
        "Statystyki cyfry"
    ],
    "Favorites": [
        null,
        "Ulubione"
    ],
    "Open For Trading": [
        null,
        "Otwarte dla handlu"
    ],
    "Closed": [
        null,
        "Zamknite"
    ],
    "Contract ID": [
        null,
        "ID kontraktu"
    ],
    "Reference ID": [
        null,
        "ID referencyjne"
    ],
    "Entry Spot": [
        null,
        "Pozycja wejciowa"
    ],
    "Entry Spot Time": [
        null,
        "Czas pozycji wejciowej"
    ],
    "Potential Payout": [
        null,
        "Potencjalna wypata"
    ],
    "Exit Spot": [
        null,
        "Pozycja wyjciowa"
    ],
    "Exit Spot Time": [
        null,
        "Czas pozycji wyjciowej"
    ],
    "Sell Price": [
        null,
        "Cena sprzeday"
    ],
    "Sell Time": [
        null,
        "Czas sprzeday"
    ],
    "Trade Again": [
        null,
        "Handluj ponownie"
    ],
    "Barrier": [
        null,
        "Limit"
    ],
    "You Won": [
        null,
        "Wygrana"
    ],
    "You Lost": [
        null,
        "Przegrana"
    ],
    "To change your name, date of birth, country of residence, or email, contact Customer Support.": [
        null,
        "Aby zmieni imi lub nazwisko, dat urodzenia, miejsce zamieszkania lub adres e-mail, skontaktuj si z dziaem obsugi klienta."
    ],
    "Cashier is locked per your request. To unlock it, enter your password.": [
        null,
        "Sekcja Kasjer zostaa zablokowana na Twoj prob. Aby j odblokowa, wpisz haso."
    ],
    "Unlock Cashier": [
        null,
        "Odblokuj sekcj Kasjer"
    ],
    "Time out until": [
        null,
        "Okres wyczenia do"
    ],
    "No transactions for the selected period": [
        null,
        "Brak transakcji w wybranym okresie"
    ],
    "You have no open contracts": [
        null,
        "Nie masz adnych otwartych kontraktw"
    ],
    "You have no assets in watchlist": [
        null,
        "Nie masz adnych aktyww na licie do obserwowania"
    ],
    "Basic": [
        null,
        "Podstawowe"
    ],
    "Digits": [
        null,
        "Cyfry"
    ],
    "Advanced": [
        null,
        "Zaawansowane"
    ],
    "Higher": [
        null,
        "Wyszy"
    ],
    "Lower": [
        null,
        "Niszy"
    ],
    "Ends Outside": [
        null,
        "Zakoczy si poza"
    ],
    "Ends Between": [
        null,
        "Zakoczy si pomidzy"
    ],
    "Stays Between": [
        null,
        "Pozostanie pomidzy"
    ],
    "Goes Outside": [
        null,
        "Przekroczy"
    ],
    "Does Not Touch": [
        null,
        "Nie osignie"
    ],
    "Credit / Debit": [
        null,
        "Winien/Ma"
    ],
    "Last 25 Ticks": [
        null,
        "Ostatnich 25 zmian cen"
    ],
    "Last 50 Trades": [
        null,
        "Ostatnie 50 zakadw"
    ],
    "Last 100 Trades": [
        null,
        "Ostatnie 100 zakadw"
    ],
    "Last 500 Trades": [
        null,
        "Ostatnie 500 zakadw"
    ],
    "Last 1000 Trades": [
        null,
        "Ostatnie 1000 zakadw"
    ],
    "Maximum Daily Turnover": [
        null,
        "Maksymalny dzienny obrt"
    ],
    "Commodities": [
        null,
        "Towary"
    ],
    "Major Pairs": [
        null,
        "Gwne pary"
    ],
    "Minor Pairs": [
        null,
        "Drugorzdne pary"
    ],
    "Smart FX": [
        null,
        "Inteligentne rynki forex"
    ],
    "Volatility Indices": [
        null,
        "Wskaniki zmiennoci"
    ],
    "Indices": [
        null,
        "Wskaniki"
    ],
    "OTC Stocks": [
        null,
        "Akcje OTC"
    ],
    "Stated limits are subject to change without prior notice": [
        null,
        "Okrelone limity mog zosta zmienione bez wczeniejszego powiadomienia"
    ],
    "Withdrawal Limits": [
        null,
        "Limity wypat"
    ],
    "Withdrawal limit": [
        null,
        "Limit wypat"
    ],
    "Already withdrawn": [
        null,
        "Ju wypacono"
    ],
    "Current immediate maximum withdrawal": [
        null,
        "Obecna maksymalna natychmiastowa wypata"
    ],
    "Create Free Account": [
        null,
        "Utwrz darmowe konto"
    ],
    "Verification Code": [
        null,
        "Kod weryfikacyjny"
    ],
    "Confirm Password": [
        null,
        "Potwierd haso"
    ],
    "Already have an account?": [
        null,
        "Masz ju konto?"
    ],
    "Enter a valid verification code": [
        null,
        "Podaj prawidowy kod weryfikacyjny"
    ],
    "Choose your country": [
        null,
        "Wybierz swj kraj"
    ],
    "Password should have lower and uppercase letters and 6 characters or more": [
        null,
        "Haso powinno skada si z co najmniej 6 znakw i zawiera mae i wielkie litery"
    ],
    "Passwords do not match": [
        null,
        "Hasa nie s zgodne"
    ],
    "Enter a valid email": [
        null,
        "Podaj prawidowy adres e-mail"
    ],
    "Number should between 10 to 120": [
        null,
        "Liczba powinna mieci si w przedziale od 10 do 120"
    ],
    "Enter your first and last name": [
        null,
        "Podaj swoje imi i nazwisko"
    ],
    "City must not be empty": [
        null,
        "Pole miasto nie moe by puste"
    ],
    "Address must not be empty": [
        null,
        "Pole adres nie moe by puste"
    ],
    "Enter a valid phone number": [
        null,
        "Podaj aktualny numer telefonu"
    ],
    "Select a secret question": [
        null,
        "Wybierz sekretne pytanie"
    ],
    "Secret answer must be at least 4 characters": [
        null,
        "Odpowied na sekretne pytanie musi skada si z co najmniej 4 znakw"
    ],
    "You need to agree to our Terms and Conditions": [
        null,
        "Musisz zaakceptowa nasz regulamin"
    ],
    "I agree to the": [
        null,
        "Akceptuj"
    ],
    "terms and conditions": [
        null,
        "regulamin"
    ],
    "Search for assets": [
        null,
        "Szukaj aktyww"
    ],
    "Choose a Payment Agent": [
        null,
        "Wybierz porednika patnoci"
    ],
    "Confirm New Password": [
        null,
        "Potwierd nowe haso"
    ],
    "First Name": [
        null,
        "Imi"
    ],
    "Last Name": [
        null,
        "Nazwisko"
    ],
    "Address First Line": [
        null,
        "Adres - cz pierwsza"
    ],
    "Address Second Line": [
        null,
        "Adres - cz druga"
    ],
    "Phone": [
        null,
        "Telefon"
    ],
    "Answer To Secret Question": [
        null,
        "Odpowied na sekretne pytanie"
    ],
    "Switch to": [
        null,
        "Zmie na"
    ],
    "Single Trade": [
        null,
        "Pojedynczy zakad"
    ],
    "Multi Trade": [
        null,
        "Multi zakad"
    ],
    "Current Password": [
        null,
        "Obecne haso"
    ],
    "New Password": [
        null,
        "Nowe haso"
    ],
    "Last 50 Ticks": [
        null,
        "Ostatnich 50 zmian cen"
    ],
    "Last 100 Ticks": [
        null,
        "Ostatnich 100 zmian cen"
    ],
    "Last 500 Ticks": [
        null,
        "Ostatnich 500 zmian cen"
    ],
    "Last 1000 Ticks": [
        null,
        "Ostatnich 1000 zmian cen"
    ],
    "Exit Full Screen": [
        null,
        "Zamknij peny ekran"
    ],
    "No digit trades for this asset": [
        null,
        "Brak zakadw opartych na cyfrach dla tego aktywa"
    ],
    "Lock Cashier": [
        null,
        "Zablokuj sekcj Kasjer"
    ],
    "Time out until date": [
        null,
        "Okres wyczenia do dnia"
    ],
    "Time out until time": [
        null,
        "Okres wyczenia do godziny"
    ],
    "Light Theme": [
        null,
        "Jasny motyw"
    ],
    "Dark Theme": [
        null,
        "Ciemny motyw"
    ],
    "Password changed successfully": [
        null,
        "Haso zostao zmienione"
    ],
    "Settings updated": [
        null,
        "Zaktualizowano ustawienia"
    ],
    "Address updated": [
        null,
        "Zaktualizowano adres"
    ],
    "Barrier must not be empty": [
        null,
        "Pole limit nie moe by puste"
    ],
    "Duration is out of range": [
        null,
        "Czas trwania przekracza dozwolony przedzia"
    ],
    "Start date invalid, it needs to be five minutes or more in the future": [
        null,
        "Data rozpoczcia jest nieprawidowa, naley wybra czas rozpoczcia za co najmniej 5 minut"
    ],
    "Time format is wrong": [
        null,
        "Bdny format czasu"
    ],
    "Lower than": [
        null,
        "Niszy ni"
    ],
    "Higher than": [
        null,
        "Wyszy ni"
    ],
    "High barrier": [
        null,
        "Grny limit"
    ],
    "Low barrier": [
        null,
        "Dolny limit"
    ],
    "Touch spot": [
        null,
        "Punkt osignicia"
    ],
    "This field is required": [
        null,
        "To pole jest wymagane"
    ],
    "Should be a valid number": [
        null,
        "Powinien to by prawidowy numer"
    ],
    "Should be between 0 and {number}": [
        null,
        "Warto powinna mieci si w przedziale od 0 do {number}"
    ],
    "Time out must be after today and cannot be more than 6 weeks.": [
        null,
        "Czas wyczenia musi si zaczyna po dniu dzisiejszym i nie moe przekracza 6 tygodni."
    ],
    "Exclude time cannot be less than 6 months and more than 5 years.": [
        null,
        "Czas wyczenia nie moe by krtszy ni 6 miesicy, ani duszy ni 5 lat."
    ]
}