...
const assetSymbolsMatchedFilter = (assets, filter) =>
    assets.reduce((symbols, asset) => {
        if (
            asset.get('submarket') === filter || asset.get('market') === filter
        ) {
...
const shownAssetIndexRowsSelector = createSelector(
    ...
    (assets, filter, assetIndex) => {
        ...
        return assetIndex.filter(a => symbols.some(x => x === a.get(0)));
    },
);
...
export const assetIndexTableSelector = createSelector(
    ...
    assetIndexRows => {
        ...
        const headers = assetIndexRows.reduce((prev, curr) => {
            const types = curr
                .get(2)
                .map(durations => durations.get(1))
                .filter(t => !!t);
            return prev.merge(types);
        }, List.of());
        const data = assetIndexRows.map(row => {
            const name = row.get(1);
            const durations = headers.map(type =>
                assetIndexRowToDuration(
                    row.get(2).find(x => x.get(1) === type),
                ),
            );
